  ...
  hpmd.data = {
    ...
    trackShows: function(currentSlide) {
        if (hpmd.data.slidesName[currentSlide-1] != null) {
          hpmd.data.trackEvent(`show-slide-${hpmd.data.slidesName[currentSlide-1]}`);
          hpmd.data.slidesName[currentSlide-1] = null;
        }
    },
    ...
    slidesForView: [],
    device: '',
    sliderInitialState: function(index, selected) {
      hpmd.data.nodes['main-screen'].style.display = 'none';
      hpmd.data.nodes.wrapper.classList.remove('hpmdf-hide-carousel');
      hpmd.data.currentSlide = index + 1;
      ...
      for (let i=0; i < hpmd.data.slides.length; i++) {
        hpmd.data.nodes.slider.appendChild(hpmd.data.slides[i])
      }

      hpmd.data.trackEvent(`show-slide-${selected}`)
      hpmd.data.slidesName[index] = null;
      // setTimeout(() => {
    ...
    init: function () {
      ...
      hpmd.data.slides = [].slice.call(document.querySelectorAll('.hpmdf-slide'))
      hpmd.data.nodes.slider.innerHTML = "";

      var buttons = [].slice.call(document.querySelectorAll('.hpmdf-button'));
      var i = 0;
      hpmd.data.animationInterval = setInterval(() => {
        if (i === buttons.length) {
          for (let j=0; j<buttons.length; j++) {
            buttons[j].classList.remove('hpmd-animate-button')
          }
          i = 0;
        } else {
          buttons[i].classList.add('hpmd-animate-button')
          i++
        }
      }, 1000)
      ...
      document.querySelector('#hpmdf-button-party').addEventListener('click', () => {
        const selected = document.querySelector('#hpmdf-button-party').getAttribute(['data-name'])
        clearInterval(hpmd.data.animationInterval);
        hpmd.data.trackEvent(`hpmd-button-${selected}`)
        hpmd.data.sliderInitialState(0, selected)
      })

      document.querySelector('#hpmdf-button-strol').addEventListener('click', () => {
        const selected = document.querySelector('#hpmdf-button-strol').getAttribute(['data-name'])
        clearInterval(hpmd.data.animationInterval);
        hpmd.data.trackEvent(`hpmd-button-${selected}`)
        hpmd.data.sliderInitialState(1, selected)
      })

      document.querySelector('#hpmdf-button-envy').addEventListener('click', () => {
        const selected = document.querySelector('#hpmdf-button-envy').getAttribute(['data-name'])
        clearInterval(hpmd.data.animationInterval);
        hpmd.data.trackEvent(`hpmd-button-${selected}`)
        hpmd.data.sliderInitialState(2, selected)
      })

      for (let i=0; i < buttons.length; i++) {
        hpmd.data.handleSwipe(buttons[i], {
          tap: function () {
            const selected = buttons[i].getAttribute(['data-name'])
            clearInterval(hpmd.data.animationInterval);
            hpmd.data.trackEvent(`hpmd-button-${selected}`)
            hpmd.data.sliderInitialState(i, selected)
          }
        });
      }
  ...
  images = [
    "./img/close.png",
    "./img/third_slide_bg.jpg",
    "./img/first_slide_bg.jpg",
    "./img/second_slide_bg.jpg",
    "./img/slide1_content_bg.png",
    "./img/slide2_content_bg.png",
    "./img/slide3_content_bg.png",
    "./img/main_logo.png",
    "./img/main_text.png",
    "./img/envy.png",
    "./img/strol.png",
  ...
  hpmd.data.handleSwipe(hpmd.data.nodes.clickarea, {
    left: function () {
      hpmd.data.checkUserPlayPrev();
      hpmd.data.showNextSlide();
    },
